...
First, we will use the seurat_obj_integrated_`r {integration_method}`_clusters_`r {resolution_list}`.rds object as generated from the third step of the pipeline in the `cell-contamination-removal-analysis` module. This object contains all samples of the project integrated by the integration method of choice as defined in the `params` along with the clusters remaining after contamination clusters are removed and the final selection of the granularity (defined as `resolution_list` in the `params`).
...
if (genome_name %in% c("GRCh38", "GRCh38ANDGRCm39")) {  # Check if genome_name is "GRCh38" or "GRCh38ANDGRCm39"
  all.markers <- all.markers %>%
    # Grepl the non-human cells
    mutate(species = "human",  # Initialize 'species' as 'human'
           species = case_when(grepl("GRCm39", gene) ~ "mouse",  # If 'gene' contains "GRCm39", set 'species' to "mouse"
    ...
    } else if (genome_name %in% c("GRCm39")) {  # Check if genome_name is "GRCm39" 
      all.markers <- all.markers %>%
...
caption_value = "Summary of number of cells per species"
print(knitr::kable(tables1, align = "lcccc", caption = caption_value))
cat("  \n</div>  \n")
cat("  \n\\pagebreak  \n")
```

## Number of cells per species and cluster

```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- count(all.markers, species, cluster)
cat("  \n<div align=\"center\" style=\"font-size:80%\">  \n")
caption_value = "Summary of number of cells per species and cluster"
print(knitr::kable(tables1, align = "lcccc", caption = caption_value))
cat("  \n</div>  \n")
cat("  \n\\pagebreak  \n")
```
