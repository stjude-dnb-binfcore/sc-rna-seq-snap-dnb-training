...
Please note that for cohorts with multiple samples, the object generated by the `02-integrative-analysis.Rmd` step will be used by default. For single-sample cohorts, however, the merged object from the `01-cell-contamination-removal.Rmd` step should be used instead (`run-cell-contamination-removal-analysis-steps-1-2-3-skip-step2.R`). You can specify this preference in the `project_parameters.Config.yaml` file by defining the `integration_method_clustering_module` variable.
