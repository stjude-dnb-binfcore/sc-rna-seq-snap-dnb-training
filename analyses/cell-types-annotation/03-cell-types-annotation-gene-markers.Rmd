...
#source(paste0(analysis_dir, "/util/dittoSeq/utils-data-edit.R"))
#source(paste0(analysis_dir, "/util/dittoSeq/utils-defaulting.R"))
#source(paste0(analysis_dir, "/util/dittoSeq/utils-getters.R"))
#source(paste0(analysis_dir, "/util/dittoSeq/utils.R"))
...
# Add metadata
#new_data <- seurat_obj@meta.data %>%
#  mutate(seurat_clusters = as.numeric(seurat_clusters)) %>%
#  left_join(markers, by = "seurat_clusters", relationship = "many-to-many")
#head(new_data)
# Add the new data back into the Seurat object (ensure cell names align)
#seurat_obj$seurat_clusters <- new_data$seurat_clusters
#seurat_obj <- AddMetaData(seurat_obj, metadata = new_data)
#head(seurat_obj@meta.data)

seurat_obj_signature <- calculate_cell_type_signature(seurat_obj = seurat_obj, 
                                            results_dir = results_dir, 
                                            gene_markers_df = marker_list, 
                                            genome_name = genome_name)

head(seurat_obj_signature@meta.data)
