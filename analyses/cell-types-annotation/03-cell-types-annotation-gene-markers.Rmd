...
# The following should be removed once the library is added to the container
# library(dittoSeq)
# https://github.com/dtm2451/dittoSeq/blob/devel/R
# Get all R script files in the directory (including subdirectories if needed)
scripts_to_source <- list.files(path = paste0(analysis_dir, "/util/dittoSeq/R"), pattern = "\\.R$", full.names = TRUE)
...
p1.after <- print(DimPlot(seurat_obj, reduction = redution_value, split.by = condition_value, group.by = 'predicted.cell.signature.ident', label = TRUE, repel = TRUE, label.size = 2.5, cols = palette) + ggtitle(glue::glue("Gene markers cell type annotations: {integration_method} integration")))
