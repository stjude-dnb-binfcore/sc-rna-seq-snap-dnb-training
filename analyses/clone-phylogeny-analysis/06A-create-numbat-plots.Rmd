...
timepoint_palette_file <- file.path(root_dir, "figures", "palettes", "timepoint_palette.tsv")
...
# To be used for timepoint annotations etc
#mypalette <- brewer.pal(12,"Paired") # colorblindFriendly = TRUE
#mypalette <- colorRampPalette(mypalette)(40)

# Read color palette
mypalette_df <- readr::read_tsv(timepoint_palette_file, guess_max = 100000, show_col_types = FALSE) 

# Define and order palette
mypalette <- mypalette_df$hex_codes 
names(mypalette) <- mypalette_df$timepoint
```
...
p1 <- ggplot(metadata_table, aes(x = UMAP_1, y = UMAP_2, color = clone_opt)) +
  ...
  scale_color_manual(values = mypal) +  # Apply custom color palette (mypal should be a predefined vector)
...
p3 <- ggplot(metadata_table, aes(x = UMAP_1, y = UMAP_2, color = .data[[cell_type_label]])) +
  ...
  scale_color_manual(values = palette) +  # Apply custom color palette (mypal should be a predefined vector)
  ggtitle(cell_type_label) +
  theme_Publication () +
  theme(legend.position = "right")
p4 <- ggplot(metadata_table, aes(x = UMAP_1, y = UMAP_2, color = timepoint)) +
  geom_point(alpha = 1, size = 1) +  # Adjust transparency if needed: alpha = 0.8; # Setting alpha to 1 for no transparency
  #geom_text_repel(aes(label = timepoint), size = 3, box.padding = 0.5) +  # Add labels with repulsion
  scale_color_manual(values = mypalette) +  # Apply custom color palette (mypal should be a predefined vector)
...
```{r plot-8, fig.width = 22, fig.height = 6, fig.fullwidth = TRUE, echo=TRUE}
fname <- paste0(numbat_plots_dir, "/", sample_name[i], "/", "plot8.png")
p <- print(p_cnv + p_cnv_x + p_cnv_y + plot_layout(ncol = 3))
ggsave(file = fname, p, width = 22, height = 6, device = "png")
```
