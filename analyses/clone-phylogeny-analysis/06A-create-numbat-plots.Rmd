...
metadata_table$cell <- rownames(metadata_table)
metadata_table <- merge.data.frame(metadata_table, numbat_obj.df, by = "cell", all = TRUE) 
```
...
umap_data <- seurat_obj@reductions$umap@cell.embeddings
########################################################################
## p_cnv
# Extract feature data from metadata
feature_data_p_cnv <- metadata_table$p_cnv

# Combine UMAP coordinates and feature data into a data frame
plot_data_p_cnv <- data.frame(UMAP_1 = umap_data[, 1], UMAP_2 = umap_data[, 2], Feature = feature_data_p_cnv)

p_cnv <- ggplot(plot_data_p_cnv, aes(x = UMAP_1, y = UMAP_2, color = Feature)) +
  geom_point(alpha = 0.8) +  # Adjust transparency if needed
  scale_color_gradient2(low = 'royalblue', mid = 'white', high = 'red3', midpoint = 0.5, name = 'Posterior') +
  ggtitle("Joint posterior probability of CNV state") +
  theme_Publication () +
...
# Extract feature data from metadata
feature_data <- metadata_table$p_cnv_x

# Combine UMAP coordinates and feature data into a data frame
plot_data <- data.frame(UMAP_1 = umap_data[, 1], UMAP_2 = umap_data[, 2], Feature = feature_data)

p_cnv_x <- ggplot(plot_data, aes(x = UMAP_1, y = UMAP_2, color = Feature)) +
  geom_point(alpha = 0.8) +  # Adjust transparency if needed
  scale_color_gradient2(low = 'royalblue', mid = 'white', high = 'red3', midpoint = 0.5, name = 'Posterior') +
...
# Extract feature data from metadata
feature_data <- metadata_table$p_cnv_y

# Combine UMAP coordinates and feature data into a data frame
plot_data <- data.frame(UMAP_1 = umap_data[, 1], UMAP_2 = umap_data[, 2], Feature = feature_data)

p_cnv_y <- ggplot(plot_data, aes(x = UMAP_1, y = UMAP_2, color = Feature)) +
  geom_point(alpha = 0.8) +  # Adjust transparency if needed
  scale_color_gradient2(low = 'royalblue', mid = 'white', high = 'red3', midpoint = 0.5, name = 'Posterior') +
...
```{r plot-4, fig.width = 20, fig.height = 6, fig.fullwidth = TRUE, echo=TRUE}
...
split_data <- metadata_table$seg  # Replace 'seg' with your splitting variable

# Combine UMAP coordinates, feature data, and the split variable into a data frame
plot_data <- data.frame(UMAP_1 = umap_data[, 1], UMAP_2 = umap_data[, 2], Feature = feature_data_p_cnv, Seg = split_data)

# Create the plot using ggplot2
p1 <- ggplot(plot_data, aes(x = UMAP_1, y = UMAP_2, color = Feature)) +
  geom_point(alpha = 0.8) +  # Adjust transparency if needed
  scale_color_gradient2(low = 'royalblue', mid = 'white', high = 'red3', midpoint = 0.5, name = 'Posterior') +
...
p2 <- ggplot(plot_data_p_cnv, aes(x = UMAP_1, y = UMAP_2, color = Feature)) +
  geom_point(alpha = 0.8) +  # Adjust transparency if needed
  scale_color_gradient2(low = 'royalblue', mid = 'white', high = 'red3', midpoint = 0.5, name = 'Posterior') +
...
p <- print(p1 + p2 + plot_layout(ncol = 2, nrow = 1))
...
metadata_table <- seurat_obj@meta.data
metadata_table$cell <- rownames(metadata_table)
metadata_table <- merge.data.frame(metadata_table, numbat_obj.df2, by = "cell", all = TRUE)

seurat_obj@meta.data <-cbind(seurat_obj@meta.data, metadata_table)
###############################################################################################################
# Customize feature plots
...
p2 <- DimPlot(seurat_obj, reduction = "umap", group.by = "compartment_opt", label = TRUE, label.size = 3 ,repel = TRUE, cols = c('royalblue', 'red3')) + 
...
```{r plot-7, fig.width = 15, fig.height = 6, fig.fullwidth = TRUE, echo=TRUE}
library(RColorBrewer)
mypalette <- brewer.pal(12,"Paired") # colorblindFriendly = TRUE
mypalette <- colorRampPalette(mypalette)(40)

p3 <- DimPlot(seurat_obj, reduction = "umap", label = TRUE, group.by = cell_type_label, repel = TRUE, label.size = 2.5, cols = mypalette) + 
...
`numbat_obj$clone_post` contains the probability of a cell belong to a clone with aneuploidy (aka being a malignant cell). We can visualize the posterior probability in a expression-based embedding:

```{r clone_post, echo=TRUE}
numbat_obj.df <- numbat_obj$clone_post 
...
##############################################################
# Extract feature data from metadata
feature_data_p_cnv <- numbat_obj.df$p_cnv
# Combine UMAP coordinates and feature data into a data frame
plot_data_p_cnv <- data.frame(UMAP_1 = umap_data[, 1], UMAP_2 = umap_data[, 2], Feature = feature_data_p_cnv)
...
##############################################################
# Extract feature data from metadata
feature_data <- numbat_obj.df$p_cnv_x
...
##############################################################
# Extract feature data from metadata
feature_data <- numbat_obj.df$p_cnv_y
...
```{r plot-8, fig.width = 20, fig.height = 6, fig.fullwidth = TRUE, echo=TRUE}
