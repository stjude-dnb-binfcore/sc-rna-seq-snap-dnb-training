...
data_dir <- file.path(root_dir, "analyses", "integrative-analysis", "results")
...
We can see that one cluster is very distinct and does not change with the value of \(k\). This is the Center players which are very different to the other positions. On the other side of the tree we see a single cluster that splits into the two clusters we would expect to see. After this the tree becomes messier and there might be even nodes with multiple incoming edges. This is a good indication that we have over clustered the data.
...
if (length(resolution_list) > 1 | is.null(resolution_list)) {
  ...
  print(clustree_overlay(seurat_obj, prefix = "RNA_snn_res.", x_value = "PC_1", y_value = "PC_2",
