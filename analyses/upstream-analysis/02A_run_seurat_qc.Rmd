...
suppressPackageStartupMessages({
  ...
  options(future.globals.maxSize = 8388608000) #8000 * 1024^2
...
pct_cells <- apply(as.data.frame(seurat_obj@assays$assay@counts), 1, function(x) (sum(x != 0))) / ncol(seurat_obj@assays$assay@counts)
gene_means <- rowMeans(as.data.frame(seurat_obj@assays$assay@counts))
...
num_genes <- apply(as.data.frame(seurat_obj@assays$assay@counts), 2, function(x) (sum(x != 0))) # number of expressed genes
libsize <- colSums(as.data.frame(seurat_obj@assays$assay@counts))
...
seurat_obj <- Process_Seurat(seurat_obj = seurat_obj, nfeatures_value = nfeatures_value, Genome = genome_name,
                             ...
                             plots_output = samples_plots_dir, use_condition_split = use_condition_split, condition = condition_value, print_pdf = print_pdf, PCA_Feature_List = PCA_Feature_List_value)
```
