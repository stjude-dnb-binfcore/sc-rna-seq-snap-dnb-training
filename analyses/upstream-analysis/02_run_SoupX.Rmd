...
SoupX_results_dir <- file.path(module_results_dir, paste0("02_SoupX"))
...
SoupX_plots_dir <- file.path(plots_dir, paste0("02_SoupX"))
...
# Read metadata
project.metadata <- read.csv(metadata_file, sep = '\t', header = TRUE)
...
for (i in seq_along(sample_name)) {
  ...
  # We will rename the barcodes in the corrected matrix to match the raw matrix
  ...
  # Create a new Seurat object from the corrected count matrix out, which we had before by replacing the counts slot
  #seurat_obj  <- CreateSeuratObject(counts = out) 
  
  # Create a new assay named "RNA_SoupX", and store the corrected counts matrix (seurat_obj@assays$RNA@counts) in the new assay. 
  # Essentially, we’re storing our corrected counts under a new assay name, so it won't overwrite the original RNA assay.
  #seurat_obj[["RNA_SoupX"]] <- CreateAssayObject(counts = seurat_obj@assays$RNA@counts)
  
  # This step ensures that the counts slot in the original RNA assay is replaced with the corrected matrix (out). 
  # The RNA assay will now have the corrected counts, which is why we're updating the RNA assay here.
  ...
  # Of course, the next thing you'll want to do is to load this corrected expression matrix 
  # into some downstream analysis tool and further analyse the data.

  #colnames(seurat_obj@assays$RNA@counts) = gsub('-1$', '', colnames(seurat_obj@assays$RNA@counts))
  #colnames(seurat_obj@assays$RNA@counts) = paste(sample_name, colnames(seurat_obj@assays$RNA@counts), sep=":")
...
print(knitr::kable(tables1, caption = "Summary of contamination fraction per sample"))
cat("  \n</div>  \n")
cat("  \n\\pagebreak  \n")
```
