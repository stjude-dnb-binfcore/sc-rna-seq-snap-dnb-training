...
cellranger_results_dir <- file.path(cellranger_module_results_dir, "03_cellranger_count_summary") 
seurat_plots_dir <- file.path(module_plots_dir, "01_Seurat_qc") 
SoupX_results_dir <- file.path(module_results_dir, "02_SoupX") 
SoupX_plots_dir <- file.path(module_plots_dir, "02_SoupX") 
...
### Expression QC
The read count matrix is required at this step, where each row corresponds to a gene and each column corresponds to a cell. To exclude low-quality or damaged cells or to identify outliers,<span style="color:blue;">r rule </span>. Three general metrics to check are  
...
tables1 <- multiqc_general_stats_df
cat("  \n<div style=\"font-size:80%\">  \n")
print(knitr::kable(tables1,  align = "lcccc", caption = "General Statistics per sample"))
cat("  \n</div>  \n")
cat("  \n\\pagebreak  \n")
```
...
```{r, fig.align = "left", results = "asis", message = FALSE, warning = FALSE, echo = FALSE}
tables1 <- multiqc_fastqc_df
cat("  \n<div style=\"font-size:80%\">  \n")
print(knitr::kable(tables1,  align = "lcccc", caption = "Summary FastQC per sample"))
cat("  \n</div>  \n")
cat("  \n\\pagebreak  \n")
```
...
We will summarize the estimated and removed background due to contamination per sample. This is an optional step, so it will be skipped or not as defined in `params`. Results are shown by sample (as ordered in the `Samples` section).
...
We will summarize the estimated and removed doublets per sample. Results are shown by sample (as ordered in the `Samples` section).
...
fig.cap_list <- list()
for (i in seq_along(sample_name)) {
  #fig.cap_list = glue::glue('\n\n\n\nscDblFinder plot for sample {sample_name}')
  fig.cap_list = glue::glue('\n\n\n')}
```
